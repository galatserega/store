...
from utils.pdf_assets import get_static_base64
...
def order_create(request):
    cart = Cart(request)
    if len(cart) == 0:
        messages.warning(request, "Спочатку додайте товар у корзину")
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            logo_b64 = get_static_base64('img/logo.png')
            ...
            html_warranty = render_to_string('orders/order_warranty.html', {
                'order': order,
                'cart': cart,
                'logo_b64': logo_b64,
            })
            ...
            pdf_file.seek(0)
            pdf_warranty = BytesIO()

            pisa.CreatePDF(html_warranty, dest=pdf_warranty, encoding='utf-8')
            pdf_warranty.seek(0)
            ...
            email.attach(f'Замовлення: {order.id}.pdf',
                         pdf_file.read(), 'application/pdf')
            email.attach(f'Гарантійний талон: {order.id}.pdf',
                         pdf_warranty.read(), 'application/pdf')
